...
2. **Gene de-replication**. At this point gene-nucleotide sequences from all samples are concatenated together and duplicated sequences are removed from the catalog. For this, genes are clustered at 95% identity and 90% coverage of the shorter gene using [CD-HIT](https://github.com/weizhongli/cdhit/wiki). The longest gene sequence from each cluster is then used as a reference sequence for this gene.
...
The fasta file generated by [CD-HIT](https://github.com/weizhongli/cdhit/wiki) will contain a representative sequence for each gene cluster. To extract protein sequences for each gene in the catalog, we first extract all the sequence identifiers from the [CD-HIT](https://github.com/weizhongli/cdhit/wiki) output file and use [seqtk](https://github.com/lh3/seqtk) ``subseq`` command to extract these sequences from ``gene_catalog_all.faa``. This file can be then used for downstream analysis (ex. KEGG annotations, see :doc:`../profiling/function`)
...
1. **Read alignment**. In the first step, (cleaned) sequencing reads are mapped back to the gene catalog using [BWA](https://github.com/lh3/bwa) aligner. Note that forward, reverse, singleton and merged reads are mapped separately and are then filtered and merged in a later step.
...
1. Gene Length Normalisation. Since longer genes in the gene catalog are more likely to recruit more reads/inserts, it is important to normalize gene counts by gene length. Here gene length is the length of the representative gene from each cluster (i.e. the gene that was used during the mapping step). For the unaccounted (-1 fraction) we use the median gene length of all the genes in the catalog.

| Gene  | Gene Length |    KO    | Sample Count |
|:------|:------------|:--------:|:------------:|
| GeneA | 100         |  K04527  |              |
| GeneB | 200         |  K01234  |              |
 | GeneC | 300         |  K01111  |              |
...
3. Cell abundance normalisation.
4. Summarize at KO level.
5. Options for downstream analyses. 
